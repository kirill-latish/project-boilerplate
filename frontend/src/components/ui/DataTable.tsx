...
import SmartPagination, {
  type PaginationData,
} from "@/components/ui/SmartPagination"
import {
  Table,
  TableBody,
  TableCell,
...
export type DataTableProps<T, TCreate, TUpdate> = {
  ...
  title?: string
  description?: string
  relationToIdMap?: Record<string, string> // For sorting relation fields
}
...
export default function DataTable<T extends Record<string, any>, TCreate, TUpdate>({
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      ...
      try {
        ...
        // If no visible columns are set, show all by default
  ...
  // Map column key to database field name for sorting
  ...
  const visibleColumnCount = Math.max(visibleColumns.size, 1) + 1 // +1 for Actions column
  ...
  // Handle ESC key to close columns modal
  useEffect(() => {
    const handleEsc = (event: KeyboardEvent) => {
              ...
              {loading ? (
                ...
                <div className="py-8 px-4">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-sm text-gray-400 text-center max-w-md px-4">
                      No {entityName.toLowerCase()} were found. Try adjusting your search or
                      filters, or create a new {entityName.toLowerCase()}.
                    </p>
                  </div>
                </div>
              ) : (
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right whitespace-nowrap min-w-[120px]">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
                        ...
                        {availableColumns
                          ...
                          .map((column) => (
                            <TableCell
                              key={column.key}
                              className={`text-sm ${
                                ["created_at", "updated_at", "published_at"].includes(
                                  column.key
                                )
                                  ? "whitespace-nowrap"
                                  : ""
